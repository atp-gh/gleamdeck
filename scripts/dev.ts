...
function log(message: string) {
  console.log(`[gleamdeck-dev] ${message}`);
}

function warn(message: string) {
  console.warn(`[gleamdeck-dev] ${message}`);
}

function debug(message: string) {
  if (DEBUG) console.log(`[gleamdeck-dev:debug] ${message}`);
}
...
<script>
(() => {
  const events = new EventSource("/__gleamdeck_events");
  ...
  events.addEventListener("error", () => {
    console.warn("[gleamdeck-dev] build failed");
  });
})();
</script>
`;
...
async function handleRequest(req: Request): Promise<Response> {
  try {
    ...
    if (url.pathname === "/__gleamdeck_events") {
